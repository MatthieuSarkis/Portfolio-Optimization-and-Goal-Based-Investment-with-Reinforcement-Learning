    ...
    for stock in stocks:
        if not os.path.exists("data/{}.csv".format(stock)):
            ...
            if data.size > 0:
                ...
                file = open(f"data/{stock}.csv").readlines()
                if len(file) < 10:
                    os.system("rm data/{}.csv".format(stock))
...
def collect_close_prices(file: str = 'data/stocks.csv') -> None:
